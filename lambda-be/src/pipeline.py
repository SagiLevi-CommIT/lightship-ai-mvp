...
Returns a result dict with output_json_path, selected_frame_paths,
and annotated_frame_paths for the API layer to serve.
"""
import json
import logging
import os
...
class PipelineResult:
    """Structured result from pipeline.process_video."""
    ...
    def __init__(self):
        ...
        self.selected_frame_paths: Dict[int, str] = {}
        self.annotated_frame_paths: Dict[int, str] = {}
        self.snapshot_timestamps: Dict[int, float] = {}
...
class Pipeline:
    ...
    def process_video(
        ...
        """Process a single video. Returns PipelineResult on success, None on failure."""
        logger.info("=" * 60)
        ...
        # Use SnapshotSelector for initial sampling (correct frame indices)
        try:
            initial_snapshots = self.snapshot_selector.select_snapshots(
                video_metadata, is_train=is_train,
            )
            if not initial_snapshots:
                logger.warning("SnapshotSelector returned empty, falling back to dense sampling")
                initial_snapshots = self._generate_all_frame_snapshots(video_metadata)
        except Exception as e:
            logger.warning("SnapshotSelector failed, using dense sampling: %s", e)
            initial_snapshots = self._generate_all_frame_snapshots(video_metadata)

        try:
            extracted_frames = self.frame_extractor.extract_frames(video_metadata, initial_snapshots)
        ...
        ts_by_idx: Dict[int, float] = {
            s.frame_idx: s.timestamp_ms for s in initial_snapshots
        }
        ...
        for snap in initial_snapshots:
            fpath = extracted_frames.get(snap.frame_idx)
            if not fpath:
                continue
            try:
                objects = self.rekognition_labeler.label_frame_from_path(
                    fpath, snap.timestamp_ms,
                    video_metadata.width, video_metadata.height,
                )
                all_frame_objects[snap.frame_idx] = objects
            except Exception as e:
                logger.warning("Rekognition failed for frame %d: %s", snap.frame_idx, e)
                all_frame_objects[snap.frame_idx] = []

        # Select best frames for LLM analysis
        frames_with_objects = {
        ...
        result.selected_frame_paths = dict(selected_frame_images)
        result.snapshot_timestamps = {idx: ts_by_idx.get(idx, 0.0) for idx in selected_indices}
        ...
        if self.hazard_assessor and selected_frame_objects:
            try:
                ...
                h_events, h_objects, h_meta = self.hazard_assessor.assess_hazards_only(
                    ...
                    video_metadata={
                        "camera": video_metadata.camera, "fps": video_metadata.fps,
        ...
        classification_result: Dict[str, Any] = {
            "video_class": "hazard_detection", "road_type": "unknown",
            "speed": "unknown", "traffic": "unknown", "weather": "unknown",
            "collision": "none", "trial_start_prompt": "", "description": "",
            "questions": [],
        }
        ...
        if self.config_generator:
            try:
                ...
                client_config = self.config_generator.generate(
                    video_class=video_class, video_metadata=video_metadata,
                    objects=all_objects, hazard_events=hazard_events,
                    classification_result=classification_result,
                )
            except Exception as e:
                logger.warning("Config generation failed: %s", e)

        # Annotate frames and track paths
        annotated_frame_paths: Dict[int, str] = {}
        try:
        ...
        result.output_json_path = output_path
    ...
    def _generate_all_frame_snapshots(
        ...
        """Dense sampling fallback with correct fps-based frame index calculation."""
        snapshots = []
        t = 0.0
        while t < video_metadata.duration_ms:
            frame_idx = int(t / 1000.0 * video_metadata.fps)
            frame_idx = min(frame_idx, max(0, video_metadata.total_frames - 1))
            snapshots.append(SnapshotInfo(frame_idx=frame_idx, timestamp_ms=t))
            t += interval_ms
        logger.info("Generated %d dense snapshots for full video", len(snapshots))
        return snapshots
